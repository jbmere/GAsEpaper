...
def rescale(ind,lcst,dj,trns,fvp):
   nc = len(ind[0])
   mis= trns[1][rmpa['D']][rmpa['P']]
   ind[0][0] = max(mis,ind[0][0])
   dt = sum([math.ceil(dj[ind[1][pos]]/fvp[0][ind[2][pos]]) for pos in range(nc)])
   if dt + sum(ind[0]) <= lcst:
      return ind[0]
   res = nc * [0]
   res[0] = ind[0][0]
   j = random.randint(0,nc-1)
   ips = ind[0][j]
   while sum(res) + ips <= lcst - dt:
      res[j] = ind[0][j]
      j = random.randint(0,nc-1)
      ips = ind[0][j]
   return res
...
def evaluate(ind,cost,dj,trns,fvp,out):
   ...
   nc = len(ind[0])
   dtf= sum(ind[0])
   ind[0] = rescale(ind,len(cost),dj,trns,fvp)
   tc,cs = 0,0
   ...
   for g in range(niter):
        ...
        for mutant in offspring:
            if random.random() < pmut:
               mutation(mutant,avail=int((T-sum(dj))/1.1),mis=trns[1][rmpa['D']][rmpa['P']],pr=pmut,ll=0,ul=nv-1)
        ...
        bestit = tools.selBest(pop,6)
        pop[0:3] = bestit[0:3]
        pop[3:] = offspring[0:min(len(offspring),size)]
        pop[len(pop):len(pop)+3] = bestit[3:6]
        # Gather all the fitnesses in one list and print the stats
        fits = [ind.fitness.values[0] for ind in pop]
        xfit = [idx for idx in fits if idx > 99999]
        valid= len(invalid_ind)-len(xfit)
        print "  Min %s " % min(fits)
        print "  Valids: %s " % valid
        # print " Gaps: %s " % [pop[idx][0] for idx in range(len(pop))]
